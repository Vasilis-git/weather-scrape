...
class OkairosSpiderSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        source = 'okairos.gr'
        city = 'Tripoli'
        # time         = datetime.datetime(int(timestr[6:10]), int(timestr[3:5]), int(timestr[0:2]),
        # int(timestr[11:13]),int(timestr[14:16]))  -  datetime.timedelta(hours=2, minutes=0)
        # hour = response.xpath('//*[@class="wnfp"]/table[1]/tr[2]/td[1]/text()').get()[0:2]
        hour = response.xpath('//*[@class="wnfp"]/table[1]//tr[3]/td[1]/text()').get()
        ...
        temperature = float(response.xpath('//*[@class="wnfp"]/table[1]/tr[3]/td[3]/div/text()').get()[:-1])
        humidity = float(response.xpath('//*[@class="wnfp"]/table[1]/tr[3]/td[9]/text()').get()[:-1])
        # windends    = (rows[6].xpath('td//text()')[4].extract()).find(" ")
        # winddire    = (rows[6].xpath('td//text()')[4].extract()).find("at")
        barometer = float(
            response.xpath('//*[@class="wnfp"]/table[1]/tr[3]/td[10]/text()').get().replace("\n", "").replace("\t", "")[:-4])
        yetos = float(
            response.xpath('//*[@class="wnfp"]/table[1]/tr[3]/td[7]/text()').get().replace("\t", "").replace(",", ".")[:-2])
        direction = 'null'
