...
class OkairosSpiderSpider(scrapy.Spider):
    name = 'okairos_spider'
    allowed_domains = [
        'https://www.okairos.gr/%CF%84%CF%81%CE%AF%CF%80%CE%BF%CE%BB%CE%B7.html?v=%CF%89%CF%81%CE%B9%CE%B1%CE%AF%CE%B1']
    start_urls = [
        'https://www.okairos.gr/%CF%84%CF%81%CE%AF%CF%80%CE%BF%CE%BB%CE%B7.html?v=%CF%89%CF%81%CE%B9%CE%B1%CE%AF%CE%B1']
    ...
    def parse(self, response):
        ...
        barometer = float(
            response.xpath('//*[@class="wnfp"]/table[1]/tr[2]/td[10]/text()').get().replace("\n", "").replace("\t", "")[
            :-4])
        yetos = float(
            response.xpath('//*[@class="wnfp"]/table[1]/tr[2]/td[7]/text()').get().replace("\t", "").replace(",", ".")[
            :-2])
        direction = 'null'
